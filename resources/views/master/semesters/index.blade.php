...
<div class="space-y-6" x-data="{ deleteModal: false, deleteUrl: '' }">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500">Kelola semester tahun ajaran</p>
        </div>
        <a href="{{ route('master.semesters.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="p-4 border-b border-gray-200">
            <form method="GET" action="{{ route('master.semesters.index') }}">
                <div class="flex flex-col sm:flex-row gap-3">
                    <div class="flex-1">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari semester..." class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                        ...
                        <th class="text-left px-4 py-3 font-medium text-gray-600">Tanggal Mulai</th>
                        <th class="text-left px-4 py-3 font-medium text-gray-600">Tanggal Selesai</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($semesters as $semester)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900">{{ $semester->type->label() }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $semester->academicYear->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $semester->start_date->format('d/m/Y') }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $semester->end_date->format('d/m/Y') }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    @unless($semester->is_active)
                                        <form method="POST" action="{{ route('master.semesters.activate', $semester) }}" class="inline">
                                            @csrf
                                    ...
                                    @endunless
                                    <a href="{{ route('master.semesters.edit', $semester) }}" class="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <button type="button" @click="deleteModal = true; deleteUrl = '{{ route('master.semesters.destroy', $semester) }}'" class="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                ...
                                <p class="text-sm text-gray-500">Belum ada data semester</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6" x-transition>
                <div class="text-center">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">Hapus Semester?</h3>
                    <p class="text-sm text-gray-500 mb-6">Data semester akan dihapus secara permanen.</p>
