...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Tambah Ruang</h3>

        <form method="POST" action="{{ route('master.rooms.store') }}" class="space-y-5">
            @csrf

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1.5">Nama Ruang</label>
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Contoh: R. TKJ 1" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('name') border-red-500 @enderror">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code') }}" placeholder="Contoh: R-TKJ-01" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('code') border-red-500 @enderror">
                    @error('code')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <select name="type" id="type" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('type') border-red-500 @enderror">
                    ...
                    <option value="regular" {{ old('type') === 'regular' ? 'selected' : '' }}>Ruang Kelas</option>
                    <option value="lab" {{ old('type') === 'lab' ? 'selected' : '' }}>Laboratorium</option>
                    <option value="office" {{ old('type') === 'office' ? 'selected' : '' }}>Ruang Guru</option>
                </select>
                @error('type')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="building" id="building" value="{{ old('building') }}" placeholder="Contoh: Gedung A" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('building') border-red-500 @enderror">
                    @error('building')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" name="floor" id="floor" value="{{ old('floor') }}" min="0" placeholder="Contoh: 1" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('floor') border-red-500 @enderror">
                    @error('floor')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
