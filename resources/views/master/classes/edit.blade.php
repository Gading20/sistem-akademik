...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Edit Kelas</h3>

        <form method="POST" action="{{ route('master.classes.update', $class) }}" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1.5">Nama Kelas</label>
                <input type="text" name="name" id="name" value="{{ old('name', $class->name) }}" placeholder="Contoh: X TKJ 1" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('name') border-red-500 @enderror">
                @error('name')
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="grade_level" id="grade_level" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('grade_level') border-red-500 @enderror">
                        ...
                        <option value="10" {{ old('grade_level', $class->grade_level) == '10' ? 'selected' : '' }}>X (10)</option>
                        <option value="11" {{ old('grade_level', $class->grade_level) == '11' ? 'selected' : '' }}>XI (11)</option>
                        <option value="12" {{ old('grade_level', $class->grade_level) == '12' ? 'selected' : '' }}>XII (12)</option>
                    </select>
                    @error('grade_level')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="section" id="section" value="{{ old('section', $class->section) }}" placeholder="Contoh: 1" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('section') border-red-500 @enderror">
                    @error('section')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="semester_id" id="semester_id" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('semester_id') border-red-500 @enderror">
                        ...
                        @foreach($semesters as $semester)
                            <option value="{{ $semester->id }}" {{ old('semester_id', $class->semester_id) == $semester->id ? 'selected' : '' }}>{{ $semester->name }}</option>
                        @endforeach
                    </select>
                    @error('semester_id')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $class->is_active) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
