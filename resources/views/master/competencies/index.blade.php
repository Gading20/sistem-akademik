...
@section('header', 'Kompetensi Keahlian')

@section('content')
<div class="space-y-6" x-data="{ deleteModal: false, deleteUrl: '' }">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-lg font-semibold text-gray-900">Kompetensi Keahlian</h2>
            <p class="text-sm text-gray-500">Kelola kompetensi keahlian per jurusan</p>
        </div>
        <a href="{{ route('master.competencies.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="p-4 border-b border-gray-200">
            <form method="GET" action="{{ route('master.competencies.index') }}">
                <div class="flex flex-col sm:flex-row gap-3">
                    <div class="flex-1">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kompetensi..." class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($competencies as $competency)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 font-mono text-sm text-gray-900">{{ $competency->code }}</td>
                            <td class="px-4 py-3 font-medium text-gray-900">{{ $competency->name }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $competency->major->name }}</td>
                            <td class="px-4 py-3 text-gray-600 max-w-xs truncate">{{ $competency->description ?: '-' }}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('master.competencies.edit', $competency) }}" class="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <button type="button" @click="deleteModal = true; deleteUrl = '{{ route('master.competencies.destroy', $competency) }}'" class="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center">
                                ...
                                <p class="text-sm text-gray-500">Belum ada data kompetensi keahlian</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6" x-transition>
                <div class="text-center">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">Hapus Kompetensi?</h3>
                    <p class="text-sm text-gray-500 mb-6">Data kompetensi akan dihapus secara permanen.</p>
