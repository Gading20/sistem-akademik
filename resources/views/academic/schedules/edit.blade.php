...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Edit Jadwal</h3>

        <form method="POST" action="{{ route('academic.schedules.update', $schedule) }}" class="space-y-5">
            @csrf
            @method('PUT')

            <input type="hidden" name="teaching_assignment_id" id="teaching_assignment_id" value="{{ old('teaching_assignment_id', $schedule->teaching_assignment_id) }}">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $schedule->start_time) }}" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('start_time') border-red-500 @enderror">
                    @error('start_time')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $schedule->end_time) }}" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('end_time') border-red-500 @enderror">
                    @error('end_time')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const currentTA = {{ $schedule->teaching_assignment_id }};
    const currentTAData = taData.find(ta => ta.id === currentTA);
    const currentTeacherId = currentTAData ? currentTAData.teacher_id : '';
    const currentClassId = currentTAData ? currentTAData.class_id : '';
    ...
    function populateClasses(teacherId, selectedClassId) {
        ...
        filtered.forEach(ta => {
            ...
            if (ta.class_id == selectedClassId) opt.selected = true;
            classSelect.appendChild(opt);
        });

        classSelect.disabled = false;

        const match = taData.find(ta => ta.teacher_id == teacherId && ta.class_id == selectedClassId);
        if (match) {
    ...
    if (currentTeacherId) {
        ...
        populateClasses(currentTeacherId, currentClassId);
    }
