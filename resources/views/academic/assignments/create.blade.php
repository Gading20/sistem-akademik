...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Tambah Tugas</h3>

        <form method="POST" action="{{ route('academic.assignments.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            ...
            @if($isGuru)
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Tugas tercatat atas nama Anda.</p>
                </div>
            @else
                <div>
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="Judul tugas" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('title') border-red-500 @enderror">
                @error('title')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Deskripsi tugas..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="deadline" id="deadline" value="{{ old('deadline') }}" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('deadline') border-red-500 @enderror">
                    @error('deadline')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" name="max_score" id="max_score" value="{{ old('max_score', 100) }}" min="0" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('max_score') border-red-500 @enderror">
                    @error('max_score')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
