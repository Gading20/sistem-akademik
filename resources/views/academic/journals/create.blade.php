...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Tambah Jurnal</h3>

        <form method="POST" action="{{ route('academic.journals.store') }}" class="space-y-5">
            @csrf
            ...
            @if($isGuru)
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Jurnal tercatat atas nama Anda.</p>
                </div>
            @else
                <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', date('Y-m-d')) }}" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('date') border-red-500 @enderror">
                    @error('date')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="material" id="material" rows="3" placeholder="Deskripsikan materi atau kegiatan pembelajaran..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none @error('material') border-red-500 @enderror">{{ old('material') }}</textarea>
                @error('material')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="activities" id="activities" rows="2" placeholder="Langkah-langkah kegiatan pembelajaran..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none @error('activities') border-red-500 @enderror">{{ old('activities') }}</textarea>
                @error('activities')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="2" placeholder="Catatan tambahan..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
