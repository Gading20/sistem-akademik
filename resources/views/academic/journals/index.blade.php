...
<div class="space-y-6" x-data="{ deleteModal: false, deleteUrl: '' }">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-lg font-semibold text-gray-900">Jurnal Mengajar</h2>
            <p class="text-sm text-gray-500">Catatan kegiatan mengajar harian</p>
        </div>
        @can('create', \App\Models\Journal::class)
        <a href="{{ route('academic.journals.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="p-4 border-b border-gray-200">
            <form method="GET" action="{{ route('academic.journals.index') }}">
                <div class="flex flex-col sm:flex-row gap-3">
                    ...
                    <div class="w-full sm:w-40">
                        <input type="date" name="date" value="{{ request('date') }}" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                        ...
                        <th class="text-left px-4 py-3 font-medium text-gray-600">Materi</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($journals as $journal)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-900">{{ \Carbon\Carbon::parse($journal->date)->locale('id')->isoFormat('D MMMM Y') }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $journal->classRoom->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $journal->subject->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-600 max-w-xs">
                                <div class="truncate" title="{{ $journal->material }}">{{ $journal->material }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    @can('update', $journal)
                                        <a href="{{ route('academic.journals.edit', $journal) }}" class="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @endcan
                                    @can('delete', $journal)
                                        <button type="button" @click="deleteModal = true; deleteUrl = '{{ route('academic.journals.destroy', $journal) }}'" class="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center">
                                ...
                                <p class="text-sm text-gray-500">Belum ada jurnal mengajar</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6" x-transition>
                <div class="text-center">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">Hapus Jurnal?</h3>
                    <p class="text-sm text-gray-500 mb-6">Data jurnal akan dihapus secara permanen.</p>
