...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6" x-data="questionForm()">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Edit Soal</h3>

        <form method="POST" action="{{ route('exam.questions.update', [$questionBank, $question]) }}" class="space-y-5" onsubmit="return cleanOptions(this)">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="type" id="type" x-model="questionType" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('type') border-red-500 @enderror">
                        @foreach(\App\Enums\QuestionTypeEnum::cases() as $qType)
                            <option value="{{ $qType->value }}" {{ old('type', $question->type) == $qType->value ? 'selected' : '' }}>{{ $qType->label() }}</option>
                        @endforeach
                    </select>
                    @error('type')
                ...
                <div>
                    ...
                    <input type="number" name="points" id="points" value="{{ old('points', $question->points) }}" min="0.01" step="0.01" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('points') border-red-500 @enderror">
                    @error('points')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="question" id="question" rows="4" placeholder="Tulis pertanyaan..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none @error('question') border-red-500 @enderror">{{ old('question', $question->question) }}</textarea>
                @error('question')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="explanation" id="explanation" rows="2" placeholder="Penjelasan singkat untuk pembahasan..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none @error('explanation') border-red-500 @enderror">{{ old('explanation', $question->explanation) }}</textarea>
                @error('explanation')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label class="flex items-center gap-2">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $question->is_active) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
...
<script>
    ...
    function questionForm() {
        const existing = @json($question->options->map(fn ($o) => ['text' => $o->option_text, 'correct' => (bool) $o->is_correct])->toArray());

        return {
            questionType: '{{ old('type', $question->type) }}',
            options: existing.length >= 2 ? existing : [{ text: '', correct: false }, { text: '', correct: false }],
