...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Buat Ujian</h3>

        <form method="POST" action="{{ route('exam.exams.store') }}" class="space-y-5">
            @csrf
            ...
            @if($isGuru)
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Ujian tercatat atas nama Anda.</p>
                </div>
            @else
                <div>
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="Judul ujian" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('title') border-red-500 @enderror">
                @error('title')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div>
                    ...
                    <select name="academic_year_id" id="academic_year_id" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('academic_year_id') border-red-500 @enderror">
                        ...
                        @foreach($academicYears as $year)
                            <option value="{{ $year->id }}" {{ old('academic_year_id', $defaultAcademicYearId) == $year->id ? 'selected' : '' }}>{{ $year->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="semester_id" id="semester_id" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('semester_id') border-red-500 @enderror">
                        ...
                        @foreach($semesters as $semester)
                            <option value="{{ $semester->id }}" {{ old('semester_id', $defaultSemesterId) == $semester->id ? 'selected' : '' }}>{{ $semester->name }}</option>
                        @endforeach
                    </select>
                    @error('semester_id')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 p-3 border border-gray-300 rounded-lg @error('class_ids') border-red-500 @enderror">
                    @foreach($classes as $class)
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="class_ids[]" value="{{ $class->id }}" {{ in_array($class->id, old('class_ids', [])) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="start_at" id="start_at" value="{{ old('start_at') }}" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('start_at') border-red-500 @enderror">
                    @error('start_at')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="datetime-local" name="end_at" id="end_at" value="{{ old('end_at') }}" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('end_at') border-red-500 @enderror">
                    @error('end_at')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', 60) }}" min="5" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('duration_minutes') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="number" name="attempt_limit" id="attempt_limit" value="{{ old('attempt_limit', 1) }}" min="1" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('attempt_limit') border-red-500 @enderror">
                    @error('attempt_limit')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" name="passing_score" id="passing_score" value="{{ old('passing_score', 60) }}" min="0" max="100" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('passing_score') border-red-500 @enderror">
                    @error('passing_score')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Instruksi ujian..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="random_question" value="1" {{ old('random_question', true) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                    <span class="text-sm text-gray-700">Acak urutan soal</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="random_option" value="1" {{ old('random_option', true) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                    <span class="text-sm text-gray-700">Acak urutan pilihan jawaban</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="shuffle_options" value="1" {{ old('shuffle_options', true) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                    <span class="text-sm text-gray-700">Acak pilihan pada soal pilihan ganda</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="show_result" value="1" {{ old('show_result') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                    <span class="text-sm text-gray-700">Tampilkan nilai setelah selesai</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="show_answer_after_submit" value="1" {{ old('show_answer_after_submit') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                            ...
                            <div class="mt-2 space-y-1.5 max-h-64 overflow-y-auto pl-1">
                                @foreach($bank->questions as $question)
                                    <label class="flex items-start gap-2 cursor-pointer rounded-lg hover:bg-gray-50 px-2 py-1.5">
                                        <input type="checkbox" name="questions[]" value="{{ $question->id }}" {{ in_array($question->id, old('questions', [])) ? 'checked' : '' }} class="mt-1 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
            ...
            <p class="text-xs text-gray-400 -mt-2">Ujian tersimpan sebagai draft dan dapat dipublikasikan dari halaman daftar ujian.</p>
        </form>
    </div>
</div>
@endsection
