...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Edit Bank Soal</h3>

        <form method="POST" action="{{ route('exam.question-banks.update', $questionBank) }}" class="space-y-5">
            @csrf
            @method('PUT')

            @if($isGuru)
                <input type="hidden" name="teacher_id" value="{{ old('teacher_id', $questionBank->teacher_id) }}">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Guru</label>
                    <input type="text" value="{{ $questionBank->teacher?->user->name ?? '-' }}" disabled class="w-full px-3 py-2.5 text-sm border border-gray-300 bg-gray-50 rounded-lg text-gray-500">
                    <p class="mt-1 text-xs text-gray-500">Bank soal tetap tercatat atas nama pemiliknya.</p>
                </div>
            @else
                <div>
                    ...
                    <select name="teacher_id" id="teacher_id" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('teacher_id') border-red-500 @enderror">
                        ...
                        @foreach($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ old('teacher_id', $questionBank->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->user->name }} ({{ $teacher->nip }})</option>
                        @endforeach
                    </select>
                    @error('teacher_id')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $questionBank->name) }}" placeholder="Contoh: Bank Soal Matematika Kelas X" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('name') border-red-500 @enderror">
                @error('name')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Deskripsi bank soal..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none @error('description') border-red-500 @enderror">{{ old('description', $questionBank->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
