...
@section('header', 'Edit Konfigurasi Penilaian')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Form Edit Konfigurasi</h3>

        <form method="POST" action="{{ route('grading.configs.update', $gradingConfig) }}" class="space-y-5">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="class_id" id="class_id" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('class_id') border-red-500 @enderror">
                        ...
                        @foreach($classes as $class)
                            <option value="{{ $class->id }}" {{ old('class_id', $gradingConfig->class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                        @endforeach
                    </select>
                    @error('class_id')
                ...
                <div>
                    ...
                    <select name="subject_id" id="subject_id" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('subject_id') border-red-500 @enderror">
                        ...
                        @foreach($subjects as $subject)
                            <option value="{{ $subject->id }}" {{ old('subject_id', $gradingConfig->subject_id) == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>
                        @endforeach
                    </select>
                    @error('subject_id')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="academic_year_id" id="academic_year_id" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('academic_year_id') border-red-500 @enderror">
                        ...
                        @foreach($academicYears as $year)
                            <option value="{{ $year->id }}" {{ old('academic_year_id', $gradingConfig->academic_year_id) == $year->id ? 'selected' : '' }}>{{ $year->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="semester_id" id="semester_id" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('semester_id') border-red-500 @enderror">
                        ...
                        @foreach($semesters as $semester)
                            <option value="{{ $semester->id }}" {{ old('semester_id', $gradingConfig->semester_id) == $semester->id ? 'selected' : '' }}>{{ $semester->type->label() }} - {{ $semester->academicYear->name }}</option>
                        @endforeach
                    </select>
                    @error('semester_id')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            ...
            <div>
                ...
                <select name="method" id="method" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('method') border-red-500 @enderror">
                    ...
                    <option value="automatic" {{ old('method', $gradingConfig->method) === 'automatic' ? 'selected' : '' }}>Otomatis</option>
                    <option value="manual" {{ old('method', $gradingConfig->method) === 'manual' ? 'selected' : '' }}>Manual</option>
                </select>
                @error('method')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="tugas_weight" id="tugas_weight" value="{{ old('tugas_weight', $gradingConfig->tugas_weight) }}" min="0" max="100" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('tugas_weight') border-red-500 @enderror">
                        @error('tugas_weight')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" name="quiz_weight" id="quiz_weight" value="{{ old('quiz_weight', $gradingConfig->quiz_weight) }}" min="0" max="100" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('quiz_weight') border-red-500 @enderror">
                        @error('quiz_weight')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" name="uts_weight" id="uts_weight" value="{{ old('uts_weight', $gradingConfig->uts_weight) }}" min="0" max="100" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('uts_weight') border-red-500 @enderror">
                        @error('uts_weight')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" name="uas_weight" id="uas_weight" value="{{ old('uas_weight', $gradingConfig->uas_weight) }}" min="0" max="100" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('uas_weight') border-red-500 @enderror">
                        @error('uas_weight')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" name="practical_weight" id="practical_weight" value="{{ old('practical_weight', $gradingConfig->practical_weight) }}" min="0" max="100" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('practical_weight') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="number" name="project_weight" id="project_weight" value="{{ old('project_weight', $gradingConfig->project_weight) }}" min="0" max="100" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none @error('project_weight') border-red-500 @enderror">
